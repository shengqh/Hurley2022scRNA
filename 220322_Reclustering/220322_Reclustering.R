...
parSampleFile3=''
parFile1='<file path to PH_scRNA.final.rds>'
parFile2='<file path to PH_scRNA.rename_cluster.csv>'
parFile3='PH_scRNA.celltype.rds'
source("scRNA_func.r")
library(Seurat)
library(ggplot2)
library(patchwork)
...
for(ct in names(cclist)){
  ...
  png(paste0(ctPrefix, ".pre.png"), width=6400, height=3000, res=300)
  pre<-DimPlot(object = cluster_obj, reduction = 'umap', label=TRUE, group.by=myoptions$cluster_name, split.by="group") + ggtitle("")
  print(pre)
  dev.off()
  ...
  png(paste0(ctPrefix, ".post.png"), width=6400, height=3000, res=300)
  post<-DimPlot(object = cluster_obj, reduction = 'umap', label=TRUE, group.by="seurat_cellactivity_clusters", split.by="group") + ggtitle("")
  print(post)
  dev.off() 
  cluster_obj@reductions$full_umap=full_umap
  png(paste0(ctPrefix, ".combined.png"), width=6400, height=9600, res=300)
  post_full<-DimPlot(object = cluster_obj, reduction = 'full_umap', label=TRUE, group.by="seurat_cellactivity_clusters", split.by="group") + ggtitle("")
